"""Continuous-CDF construction for Metaculus numeric/discrete questions.

The classes below (NumericDefaults, Percentile, NumericDistribution) are
copied verbatim from the official Metaculus bot template:
https://github.com/Metaculus/metac-bot-template (main_with_no_framework.py)
which itself mirrors forecasting-tools' NumericDistribution. They implement
Metaculus' CDF standardization rules (201 points, min step 5e-05, max PMF
0.2, open/closed bound handling, log-scaled zero_point support).

Only the thin `percentiles_to_cdf` helper at the bottom is ours.
"""
...
def percentiles_to_cdf(
    ...
    """Convert {percentile(0-100): value} into a Metaculus-ready CDF list."""
    percentiles = [
